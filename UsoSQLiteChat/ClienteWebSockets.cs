...
namespace UsoSQLiteChat
{
    class ClienteWebSocketsFuncionaTambien
    {
        ...
        public static async Task Salir(string usuario)
        {
            try
            {
                Dictionary<string, string> dictionary = new Dictionary<string, string>{
                ...
                string jsonObj = JsonConvert.SerializeObject(dictionary, Formatting.Indented);
                await Task.WhenAll(enviarMensaje(jsonObj));
                await webSocket.CloseAsync(WebSocketCloseStatus.NormalClosure, "cierre de conexion", CancellationToken.None);
                webSocket.Dispose();
                webSocket = null;
            }
            catch (Exception ex)
